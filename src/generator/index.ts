...
export class AIGenerator {
  ...
  async generateAll(context: GenerationContext): Promise<GeneratedOutputs> {
    ...
    const hooks: HookOutput[] = [{
      filename: 'context-loader.sh',
      content: this.generateContextLoaderHook(context.selectedAgents, context.selectedSkills),
      hookName: 'context-loader'
    }];
...
`;
  ...
  private generatePlaceholderSkill(name: string, context: GenerationContext): string {
    return `# ${name} Skill
...
`;
  ...
  private generateContextLoaderHook(agents: string[], skills: string[]): string {
    const agentsList = agents.map(a => `   - ${a}`).join('\n');
    const skillsList = skills.map(s => `   - ${s}`).join('\n');

    return `#!/bin/bash
# SuperAgents: Context Loading Hook
# Requires explicit agent/skill decisions before implementation

cat <<'EOF'
REQUIRED: CONTEXT LOADING PROTOCOL

Before writing any code, complete these steps in order:

1. SCAN each agent below and decide: USE or SKIP (with brief reason)
${agentsList}

2. SCAN each skill below and decide: LOAD or SKIP (with brief reason)
${skillsList}

3. For agents marked USE → spawn via Task tool with appropriate subagent_type
   For skills marked LOAD → invoke Skill(name)
   If none needed → write "Proceeding without context loading"

4. Only after step 3 completes may you begin implementation.

IMPORTANT: Skipping step 3 invalidates steps 1-2. Always invoke tools for marked items.

Sample output:
Agents:
- ${agents[0] || 'backend-engineer'}: USE - implementing API logic
- ${agents[1] || 'frontend-specialist'}: SKIP - no UI work

Skills:
- ${skills[0] || 'typescript'}: LOAD - writing TypeScript code
- ${skills[1] || 'nodejs'}: SKIP - not needed

Then call:
> Task(subagent_type="${agents[0] || 'backend-engineer'}", prompt="...")
...
`;
  ...
  private generateClaudeMd(context: GenerationContext): string {
