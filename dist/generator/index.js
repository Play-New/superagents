...
`;
     ...
     */
    async executePrompt(prompt, context, model) {
        if (context.authMethod === 'claude-plan') {
            ...
            // Use streaming in verbose mode for real-time output
            if (context.verbose) {
                return await this.executeWithStreaming(anthropic, prompt, model);
            }
     ...
     */
    async executeWithStreaming(anthropic, prompt, model) {
        log.debug('Using streaming mode for generation');
        let fullContent = '';
        let chunkCount = 0;
        ...
        stream.on('text', (text) => {
            fullContent += text;
            chunkCount++;
            // Show progress every 10 chunks (to avoid too much output)
            if (chunkCount % 10 === 0) {
                log.debug(`Streaming: ${fullContent.length} chars received`);
            }
        });
        // Wait for completion
        const finalMessage = await stream.finalMessage();
        log.debug(`Streaming complete: ${fullContent.length} chars total`);
        // Return the full content (finalMessage also contains this)
