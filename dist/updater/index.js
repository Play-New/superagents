...
export class ConfigUpdater {
     ...
     */
    async promptUpdateOptions(existing, available) {
     ...
     */
    async applyUpdates(context, updates) {
        const agentsDir = path.join(this.claudeDir, 'agents');
        const skillsDir = path.join(this.claudeDir, 'skills');
        // Generate new content if needed
        const addedAgentNames = [];
        const addedSkillNames = [];
    ...
    async updateSettings(updates) {
        ...
        if (Array.isArray(settings.agents)) {
            ...
            // Remove agents
            settings.agents = settings.agents.filter(a => !updates.agentsToRemove.includes(a));
        }
        // Update skills list
