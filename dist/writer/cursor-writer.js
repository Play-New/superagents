...
export class CursorWriter {
    ...
    async writeAll(outputs) {
        ...
        await fs.writeFile(path.join(rulesDir, 'project.mdc'), projectContent, 'utf-8');
        // Write agents
        for (const agent of outputs.agents) {
            ...
            await fs.writeFile(path.join(rulesDir, 'agents', filename), agentContent, 'utf-8');
        }
        // Write skills
        for (const skill of outputs.skills) {
            ...
            await fs.writeFile(path.join(rulesDir, 'skills', filename), skillContent, 'utf-8');
        }
        return {
...
//# sourceMappingURL=cursor-writer.js.map