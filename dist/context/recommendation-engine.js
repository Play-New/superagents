...
export class RecommendationEngine {
    ...
    static TECH_KEYWORDS = {
        ...
        fundraising: {
            skills: ["fundraising", "financial-planning"],
            agents: ["cfo"],
        },
    ...
    recommend(goal, codebase) {
        ...
        // Sort by score
        const agents = Array.from(agentScores.values()).sort((a, b) => b.score - a.score);
        const skills = Array.from(skillScores.values()).sort((a, b) => b.score - a.score);
    ...
    buildProjectSpecificReason(agentName, codebase, goal) {
        ...
        switch (agentName) {
            ...
            case "devops-specialist": {
                if (depNames.has("docker") ||
                    codebase.detectedPatterns.some((p) => p.description.toLowerCase().includes("docker"))) {
